# looking to make environmental PCA
...
length(unique(lhf_gt_pc_st2_meta$acc))
nrow(lhf_gt_pc_st2_meta)

# remove rows without paleo data 
lhf_gt_pc_st2_meta <- lhf_gt_pc_st2_meta[,c(-31)] %>%
  filter(!is.na(mean_T_yr_paleo)) %>%
  distinct()

# run a PCA on the environmental variables
env_pca <- prcomp(
  x = lhf_gt_pc_st2_meta[,c(29,31:41)] %>% filter(!is.na(mean_T_yr_paleo)), 
...
lhf_gt_meta_envpc <- lhf_gt_pc_st2_meta 

# add in environmental PCs
lhf_gt_meta_envpc$envPC1 <- env_pca$x[,1]
lhf_gt_meta_envpc$envPC2 <- env_pca$x[,2]
lhf_gt_meta_envpc$envPC3 <- env_pca$x[,3]
lhf_gt_meta_envpc$envPC4 <- env_pca$x[,4]

# reorder columns 
lhf_gt_meta_envpc <- lhf_gt_meta_envpc %>%
  select(acc:lai_paleo,envPC1:envPC4, pos_154:pos_16592)

names(lhf_gt_meta_envpc)

write.csv(
  x = lhf_gt_meta_envpc, 
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/genotypes_contempory_paleo_envpcs_raw.csv"
)

# runnning glms to compare with 1st env PC
glm_scores_envpcs <- core_GLM_chi_scorer(
  input = lhf_gt_meta_envpc, 
  start_col = 46, 
...
write.csv(
  ...
  file = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/supp_glm_scores_envpcs_rCRS_03_2025.csv"
)
...
ggplot2::ggsave(
  filename = "/Users/finleythomas/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Results/lhf/figures/supplementary/latprecip_vs_envpc1_manhattan_03_2025.png",
  plot = last_plot(),
  bg = "transparent", 
  width = 11, 
  height = 7
